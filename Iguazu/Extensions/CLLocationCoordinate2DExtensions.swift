...
private extension Double {
  ...
  var inDegrees: CLLocationDegrees {
...
public extension CLLocationCoordinate2D {
  ...
  func coordinate(at distance: CLLocationDistance, direction: CLLocationDirection) -> CLLocationCoordinate2D {
    ...
    let angularDistanceLat = distance/LocationConstants.metersPerRadianLat
    let angularDistanceLon = distance/LocationConstants.metersPerRadianLon
    let bearingInRad = DegreesToRadians(direction)

    let lat2InRad = asin( sin(latInRad)*cos(angularDistanceLat) + cos(latInRad)*sin(angularDistanceLat)*cos(bearingInRad) )
    let lng2InRad = lngInRad + atan2(sin(bearingInRad)*sin(angularDistanceLon)*cos(latInRad), cos(angularDistanceLon)-sin(latInRad)*sin(lat2InRad));
...
struct LocationConstants {
  static let metersPerRadianLat: CLLocationDistance = 6373000.0
  static let metersPerRadianLon: CLLocationDistance = 5602900.0
}
